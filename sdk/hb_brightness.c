...
/* Floored n/d without pulling in libgcc's divmod. */
static unsigned int udiv(unsigned int n, unsigned int d)
{
    unsigned int q = 0;
    while (n >= d) { n -= d; q++; }
    return q;
...
void hb_brightness_set_percent(int percent)
{
    ...
    if (percent <= 40)
        level9 = BRIGHTNESS_9BIT_MIN +
                 udiv((unsigned)percent *
                      (BRIGHTNESS_9BIT_DEFAULT - BRIGHTNESS_9BIT_MIN), 40);
    else
        level9 = BRIGHTNESS_9BIT_MAX -
                 udiv((unsigned)(100 - percent) *
                      (BRIGHTNESS_9BIT_MAX - BRIGHTNESS_9BIT_DEFAULT), 60);
