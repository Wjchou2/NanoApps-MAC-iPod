 ...
 *
 * Must be used in FS mode (after start eject) only if the app
...
{
    /* Exit gestures: volume is the backwards-compat exit, Home is the primary
       one. A Home+Power combo is ignored so it never counts as a Home exit. */
